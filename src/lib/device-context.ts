...
// ============================================
// 1. DEVICE POSTURE INTERFACE & COLLECTION
// ============================================
...
export async function collectDevicePosture(): Promise<Partial<DevicePosture>> {
  ...
  // Check if already collected and cached
  const cached = localStorage.getItem('devicePosture');
  if (cached) {
    try {
      return JSON.parse(cached);
    } catch {
      // Invalid cache, continue collecting
    }
  }
  ...
  localStorage.setItem('devicePosture', JSON.stringify(posture));

  return posture;
}

// ============================================
// 2. ACCESS CONTEXT INTERFACE & COLLECTION
// ============================================
...
export async function collectAccessContext(): Promise<Partial<AccessContext>> {
  ...
  // Check if already collected and cached (but refresh IP info periodically)
  const cached = localStorage.getItem('accessContext');
  const cacheTime = localStorage.getItem('accessContextTime');
  const now = Date.now();
  ...
  let coords: { latitude?: number; longitude?: number } = { latitude: undefined, longitude: undefined };
  try {
  ...
  localStorage.setItem('accessContextTime', now.toString());

  return context;
}

// ============================================
// 3. DEVICE CONTEXT MANAGER (Main Entry Point)
// ============================================

export interface CollectedDeviceContext {
...
/**
 * Collect both device posture and access context
 * Called during login to gather all device information
 */
export async function collectDeviceContext(): Promise<CollectedDeviceContext> {
  const [devicePosture, accessContext] = await Promise.all([
    collectDevicePosture(),
    collectAccessContext(),
  ]);

  const context: CollectedDeviceContext = {
    devicePosture,
    accessContext,
    collectedAt: new Date().toISOString(),
  };

  // Store in localStorage for API interceptor to use
  localStorage.setItem('deviceContext', JSON.stringify(context));

  console.log('📱 Device Context Collected:', context);

  return context;
}

/**
 * Retrieve stored device context
 */
export function getStoredDeviceContext(): CollectedDeviceContext | null {
  if (typeof window === 'undefined') return null;

  const stored = localStorage.getItem('deviceContext');
...
/**
 * Clear stored device context (on logout)
 */
export function clearDeviceContext(): void {
  if (typeof window === 'undefined') return;

  localStorage.removeItem('deviceContext');
  localStorage.removeItem('devicePosture');
  localStorage.removeItem('accessContext');
  localStorage.removeItem('accessContextTime');
}
...
function guessDiskEncryption(os: string): boolean {
  ...
  // Windows 10+ with NTFS usually has encryption support
  if (os === 'Windows') return true;

  // For others, we can't detect reliably
  return false;
}
...
async function getRealIPInfo(): Promise<any> {
  try {
    ...
    console.error('Failed to get IP info from ipapi.co:', error);

    // Fallback to ipify
    try {
      const response = await fetch('https://api.ipify.org?format=json');
      if (!response.ok) throw new Error('Fallback IP request failed');
      return { ip: (await response.json()).ip };
    } catch {
...
async function checkImpossibleTravel(
  ...
  if (distance > 500 && hoursDiff < 2) {
    console.warn(
      `🚨 Impossible travel detected: ${distance.toFixed(0)}km in ${hoursDiff.toFixed(1)}h`
    );
    return true;
  }
